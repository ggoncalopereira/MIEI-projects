...
CatProds readproducts(CatProds CatProds,FILE *fd);

/** main cria as estruturas, chama inicializaçoes, le dos ficheiros que lhe sao dados (Clientes seguido do ficheiro de CatProds seguido do ficheiro de Vendas) sorteia os clientes e os CatProds, e depois imprime tudo**/
...
/*	vendas=init_ve(&vendas);
	...
	if (fd) {
	before=clock();
	catprods=readproducts(catprods,fd);
	after=clock();
	printf("Read P:%.2Lf\n",(long double)(after-before));
	...
	if (fd) {
	before=clock();
	cnj=prodsPorLetra(catprods,'A');	
	after=clock();
	printf("Print P:%.2Lf\n",(long double)(after-before));
	printConjunto(cnj);
	fclose(fd);
	printf("\nClientes has %d elements\nCatProds has %d elements\n",elementosClientes(clientes),totalProdutos(catprods));

	exitst=0;
/*	if (!exitst) {
	exitst=write_factura(&facturacao,&clientes,&CatProds,&vendas);
...
*/
	...
	removeCatProds(catprods);
	}
	destroiClientes(clientes);
	}}}}}}
	else
		printf("Usage: (Clientes-path) (CatProds-path) (Vendas-path)\n");
	return exitst;
}
...
/*Facturacao init_fact(Facturacao *facturacao){
	...
	if (facturacao->compras_cl) {
		facturacao->compras_pr=(int *)malloc(sizeof(*(facturacao->compras_pr))*SIZE_CatProds);
		if (facturacao->compras_pr) {
			zeroarrayi(facturacao->compras_cl,SIZE_CLIENTES);
			zeroarrayi(facturacao->compras_pr,SIZE_CatProds);
		}
		else {
...
*/
/*int print_facturacao(Facturacao *facturacao,Clientes *clientes,CatProds *CatProds) {
	int error=0;	
	FILE *fd;
	...
	if (!fd) {
		...
		print_fact_compras_pr(fd,facturacao,CatProds);	
		fclose(fd);	
	}
...
void print_fact_codigos(FILE *fd,Facturacao *facturacao) {
	...
	fprintf(fd,"\n\n");	
	for(i=0;i<26;i++)
		fprintf(fd,"CatProds começados por %c: %d\n",(char)(i+65),facturacao->codigos_pr[i]);
	fprintf(fd,"\n\n");
}

void print_fact_compras_pr(FILE *fd,Facturacao *facturacao,CatProds *CatProds) {
	int i;	
	for (i=0;i<CatProds->fill;i++) {
		fprintf(fd,"Compras do produto %s: %d\n",CatProds->CatProds+i*SIZE_PRODUTO,facturacao->compras_pr[i]);
	}
	fprintf(fd,"\n\n");
}
...
CatProds readproducts(CatProds catprods,FILE *fd) {
	...
	for(;i<SIZE_PRODUTOS && fgets(buffer,SIZE_BUFFER_PR,fd);) {				
		token=strtok(buffer," \r\n");
		p=criaProduto(token);
		if (p && !existeProduto(catprods,p))
			catprods=insereProduto(catprods,p);			
...
/*int readsales(Vendas *vendas,char const *path,Clientes *clientes,CatProds *CatProds) {
	int error=0,i=0;
	FILE *fd;	
	fd=fopen(path,"r");
	if (fd) {
		i=read_line_vendas(vendas,fd,clientes,CatProds);	
		if (!i) {
...
**/
/*int read_line_vendas(Vendas *vendas,FILE *fd,Clientes *clientes,CatProds *CatProds) {
	int i,error;	
	char buffer[SIZE_BUFFER_VE],*token=NULL;
	for(i=error=0;i<SIZE_VENDAS && fgets(buffer,SIZE_BUFFER_VE,fd);) {					
			token=strtok(buffer," \r\n");
			error=read_token_pr(token,vendas,i,CatProds);
			if (!error){
			token=strtok(NULL," \r\n");
...
*/
/** função que verifica se o produto lido consta da lista de CatProds atraves de um binary search no
 *  array previamente ordenado de CatProds ordenado pelo qsort na main
**/
/*char read_token_pr(char *token,Vendas *vendas,int i,CatProds *CatProds) {
	char error=-1;
	char *produto=bsearch_pr(token,CatProds,0,CatProds->fill-1);
	if (produto) {
...
/*int write_factura(Facturacao *facturacao,Clientes *clientes,CatProds *CatProds,Vendas *vendas) {
*/	/*Código de 'A' é 65 em ASCII, esta funcao vai buscar frequentemente o primeiro carater de um cliente ou produto e transforma-o num indice, para contar quantidades de CatProds começados por dada letra*/	
/*	int error=0,i,quanti=0;
	...
	for(i=0;i<vendas->fill;i++) {
		...
		facturacao->compras_pr[(vendas->vendas[i].produto-CatProds->CatProds)/SIZE_PRODUTO]++;
	}
	for(i=0;i<clientes->fill;i++)
		facturacao->codigos_cl[clientes->clientes[i*SIZE_CLIENTE]-65]++;
	for(i=0;i<CatProds->fill;i++)
		facturacao->codigos_pr[CatProds->CatProds[i*SIZE_PRODUTO]-65]++;
	error=print_facturacao(facturacao,clientes,CatProds);	
	return error;
}
*/
