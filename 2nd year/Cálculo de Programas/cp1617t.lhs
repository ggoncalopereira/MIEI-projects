...
{- Done -}
inv :: Float -> Int -> Float
inv x n = let (a,b) = aux n in a
          where
            aux 0 = (1,(1-x))
            aux (n+1) = let (a,b) = aux n
                        in (a+b, b*(1-x)) 

{-Teste de validade-}
prop_inv :: Int -> Property
prop_inv n = forAll(Test.QuickCheck.choose(1.0000000001,1.9999999999)) $ \x -> (isValid (inv x n) x)

isValid :: Float -> Float -> Bool
isValid x y = abs(x-y) > 0.001
\end{code}
...
worker = cataList(either (const (True,0)) (count))
        where sep c = ( c == ' ' || c == '\n' || c == '\t')
              count (x,(False,n)) = (sep x, n)
              count (x,(b,n)) = if b && not (sep x) then (sep x, n+1)
                                                    else (sep x, n)

prop_WC :: [Char] -> Bool
prop_WC x = wc_w x == wc_w_final x
...
\begin{code}
inB_tree :: Either () (B_tree b, [(b,B_tree b)]) -> B_tree b
...
--assumindo que recB_tree f = baseB_tree id f
...
{- Missing convertion to catamorphisms -}

inordB_tree :: B_tree a -> [a]
inordB_tree = cataB_tree (either (nil) join) 
              where join ( a, ((b,c):t)) = a ++ [b] ++ c ++ joinAux t
                    join ( a, []) = a
                    joinAux [] = []
                    joinAux ((b,c):t) = [b] ++ c ++ joinAux t

largestBlock :: B_tree a -> Int
largestBlock b = maximum(largestBlockLength b)

largestBlockLength :: B_tree a -> [Int]
largestBlockLength Nil = []
largestBlockLength (Block a ((b,c):t)) =[1+length t] ++ largestBlockLength a ++ largestBlockAux t ++ largestBlockLength c
                                    where largestBlockAux [] = []
                                          largestBlockAux ((b,c):t) = [1+length t] ++ largestBlockLength c

mirrorB_tree :: B_tree a -> B_tree a
mirrorB_tree Nil = Nil 
mirrorB_tree (Block a ((b,c):t)) = (Block (mirrorB_tree c) (reverse ((b,mirrorB_tree a):aux t) ))
              where aux ((b,c):t) = (b,mirrorB_tree c): aux t
                    aux [] = []
...
\begin{code}
anaA :: (c -> Either Null (c, d)) -> (d -> Either Null c) -> c -> A
anaA ga gb = inA . (id -|- anaA ga gb >< anaB ga gb) . ga

anaB :: (c -> Either Null (c, d)) -> (d -> Either Null c) -> d -> B
...
generateAlgae = (anaA ga gb)
    where ga 0 = Left()
          ga n = Right(pred n, pred n)
          gb 0 = Left()
          gb n = Right(pred n)

showAlgae :: Algae -> String
showAlgae = cataA (either (const "A") (uncurry (++)))
                  (either (const "B") id)

--prop_Algae ::  Bool
--prop_Algae = length.(showAlgae.(generateAlgae)) == fib.succ 
...
permuta :: [a] -> IO [a]
permuta [] = return []
permuta xs = do (rand_elem, new_xs) <- getR xs
                rand_xs <- permuta new_xs
                return (rand_elem : (rand_xs))


eliminatoria :: LTree Equipa -> Dist Equipa
eliminatoria = cataLTree (either return gJogo)
          where gJogo (x, y) = do equipa1 <- x
                                  equipa2 <- y
